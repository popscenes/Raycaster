...
void drawRays()
{
	for (int i = 0; i < NUM_RAYS; i++)
	{
		drawLine(player.x*MINI_MAP_SCALE, player.y * MINI_MAP_SCALE, rays[i].wallHitX * MINI_MAP_SCALE, rays[i].wallHitY * MINI_MAP_SCALE, 0xFFFF0000);
	}
}
...
void castRay(int column, float angle)
{
	...
	rays[column].wasHitVertical = vertHit;
