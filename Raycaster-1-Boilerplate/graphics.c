...
{	
	if (x < 0 || (x + width) > WINDOW_WIDTH || y < 0 || (y + height) > WINDOW_HEIGHT)
	{
		return;
	}
	...
	for (int ypos = y; ypos <= (y + height); ypos++)
	{
		...
		{
			colourBuffer[(ypos * WINDOW_WIDTH) + xpos] = colour;
		}
	}
}
...
void drawLine(int x0, int y0, int x1, int y1, uint32_t colour)
{
	...
	{
		colourBuffer[(int)(round(currentY) * WINDOW_WIDTH) + (int)round(currentX)] = colour;
		currentX += xinc;
		currentY += yInc;
	}
}
...
bool initialiseWindow() {
	...
	colourBuffer = (uint32_t*)malloc((uint32_t)WINDOW_HEIGHT * (uint32_t)WINDOW_WIDTH * sizeof(uint32_t));
	colourBufferTexture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_ABGR8888, SDL_TEXTUREACCESS_STREAMING, WINDOW_WIDTH, WINDOW_HEIGHT);
...
{
	SDL_UpdateTexture(colourBufferTexture, NULL, colourBuffer, (int)((uint32_t)WINDOW_WIDTH * sizeof(uint32_t)));
